...
class UncertaintyWithLossFeature(nn.Module):
    def __init__(self, class_num, visual_feature_dim):
        ...
        self.mask_loss_net = nn.Sequential(
            nn.Linear(in_features=1, out_features=self.embedding_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.embedding_dim, out_features=self.embedding_dim),
            nn.ReLU()
        ...
        self.predictor = nn.Sequential(
            nn.Linear(in_features=self.embedding_dim * 5, out_features=self.embedding_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.embedding_dim, out_features=3),
        )
        self.init_weight()
    ...
    def forward(self, ious, probs, cls_loss, reg_loss, mask_loss):
        ...
        mask_loss_feature = self.mask_loss_net(mask_loss.view(mask_loss.shape[0], 1))
        non_visual_input = torch.cat((iou_feature, probs_feature, cls_loss_feature, reg_loss_feature, mask_loss_feature), dim=1)
        prediction = self.predictor(non_visual_input)
        return prediction

@DETECTORS.register_module
class TwoStageDetectorSWNMask(BaseDetector, RPNTestMixin, BBoxTestMixin,
                                                              MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 transformer=None):
        super(TwoStageDetectorSWNMask, self).__init__()
        ...
        self.uncertainty_mask_weight = self.transformer_cfg.uncertainty_mask_weight
    ...
    def init_weights(self, pretrained=None):
        super(TwoStageDetectorSWNMask, self).init_weights(pretrained)
