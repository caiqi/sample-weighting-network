...
class FCNMaskHeadV2(nn.Module):
    ...
    def __init__(self,
        ...
        super(FCNMaskHeadV2, self).__init__()
    ...
    def loss(self, mask_pred, mask_targets, labels, sample_weights=None):
        loss = dict()
        if self.class_agnostic:
            ...
            loss_mask = F.binary_cross_entropy_with_logits(
                pred_slice, mask_targets, reduction="none")
            if sample_weights is None:
                return loss_mask
            sample_weights = sample_weights.view(-1, 1, 1)
            loss_mask = (loss_mask * sample_weights).mean()
