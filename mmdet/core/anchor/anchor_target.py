...
def anchor_target_single(flat_anchors,
    ...
    if not inside_flags.type(torch.uint8).any():
        return (None,) * 6
    ...
    if unmap_outputs:
        ...
        pos_inds = unmap(unmap(torch.ones_like(pos_inds), anchors.shape[0], pos_inds), num_total_anchors,
                         inside_flags).nonzero()
        neg_inds = unmap(unmap(torch.ones_like(neg_inds), anchors.shape[0], neg_inds), num_total_anchors,
                         inside_flags).nonzero()
